    ...
    <template v-if="width > 820">
      <template v-for="({ href, text, icon, dropbox, dropboxItems }) in props.navLinks" :key="href">
        <NavLinkItem
          ...
          class="linkTransition inline-block before:rounded-0.5 before:bg-current before:h-0.75 before:w-0 before:content-empty before:transform-translate-x--50% before:bottom-0 before:left-50% before:absolute hover:before:w-60%"
        />
