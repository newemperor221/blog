...
<aside class='widgets bg-body-bg-shadow border-grey-3 my-8 px-4 py-4 border-b border-t flex gap-4 justify-around z-1'>
  <!-- Random Posts Widget -->
      ...
      <ul class='post-list m-0 p-0 list-none'>
        {#each randomPosts as post}
          <li class='item border-grey-4 pb-2 pl-8 border-b border-dashed relative'>
            <a href={`/posts/${getPostSlug(post)}`} class='hover:text-color-link text-inherit no-underline flex flex-col transition-colors'>
              <span class='breadcrumb text-sm font-semibold m-0 max-h-6'>{post.data.title}</span>
              <span class='text-grey-5 text-xs mt-1 max-h-8'>{truncateText(post.data.description || post.body || '')}</span>
            </a>
          </li>
        {/each}
      </ul>
  ...
  {#if enableRecentComments && (hasWaline || hasTwikoo)}
    <div class='rpost px-4 py-4 w-1/2'>
      ...
      <ul id='new-comment' class='leancloud-recent-comment post-list m-0 p-0 list-none'>
        {#if recentComments.length > 0}
          {#each recentComments as comment}
            <li class='item border-grey-4 pb-2 pl-8 border-b border-dashed relative'>
              <a href={comment.href} class='hover:text-color-link text-inherit no-underline flex flex-col transition-colors'>
                <span class='breadcrumb text-sm font-semibold m-0 max-h-6'>{comment.nick} @ {comment.time}</span>
                <span class='text-grey-5 text-xs mt-1 max-h-8'>{truncateText(comment.text)}</span>
              </a>
            </li>
          {/each}
        {:else}
